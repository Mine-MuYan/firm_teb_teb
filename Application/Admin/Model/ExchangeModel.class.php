...
class ExchangeModel extends Model{
    protected $pk = "id";
    protected $tableName =  'daili_exchange_setting';
    ...
    {
        $exchangeObj = $this->select();
        foreach ($exchangeObj as $k => $item) {
            if ($item['id'] == 1){
                $exchangeObj[$k]['rank_name'] = "一级兑换中心";
            }elseif($item['id'] == 2){
                $exchangeObj[$k]['rank_name'] = "二级兑换中心";
            }else{
                $exchangeObj[$k]['rank_name'] = "三级兑换中心";
            }
        }
        return $exchangeObj;
    }
