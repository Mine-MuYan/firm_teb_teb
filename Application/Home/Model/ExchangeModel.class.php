...
class ExchangeModel extends Model{
    protected $pk = "id";
    protected $tableName = "daili_exchange";
    public $classOne;
    public $classTwo;
    public $allMessages;
    /**
     ...
     * @return bool|mixed
     */
    public function getExchangeMsg($maps)
    {
        $exchange = $this->where($maps)->find();
        if ($exchange){
            return $exchange;
        }else{
            return false;
    ...
    public function getUserId($service_number){
        ...
        $result = $this->field('apply_id,exchange_class,superior_number')->where($map)->find();
        return $result;
    }
    /**
     * 获取直属一级服务中心
    ...
    public function getClassOne($superior_number){
        if($superior_number != 0){
            ...
            if(!empty($service_message)){
                if($service_message['exchange_class'] == 1){
    ...
    public function getClassTwo($superior_number){
        if($superior_number != 0){
            $service_message = $this->where(array('id'=>$superior_number))->find();
            if(!empty($service_message)){
                if($service_message['exchange_class'] < 3){//1.一级兑换中心 2.二级兑换中心
                    if($service_message['exchange_class'] == 2){
