...
//            $code = 2;//不可以注册
//        }elseif(time() > $start_time && time() < $end_time && $day_achievement >= 200000){
//            $code = 2;//不可以注册
//        }else{
//            $code = 1;
//        }
        if($day_achievement >= 200000){
            $code = 2;
        }else{
            $code = 1;
        }
...
//        }
        ...
        $exchangeMsg = $exchange -> getExchangeMsg($maps);
        $userMsg = $user -> getUserMassage($_SESSION['uid']);
        $this -> assign("achievement",$achievement);
        $this -> assign("exchangeMsg", $exchangeMsg);
                ...
                if ($msg == 1) {
                    ...
                    if (empty($refereeMsg['service_number'])){
                        ...
                        if (!$bool) {
                            ...
                            if ($reduce) {
                                //兑换中心流水
                                $exchangeFlow['pay_id'] = $_SESSION['uid'];
                                $exchangeFlow['rec_id'] = 1;
                                $exchangeFlow['type'] = 4;
                                $exchangeFlow['money'] = $rankMsg['need_money'];
                                $exchangeFlow['remarks'] = "代缴用户" . $_POST['mobile'] . "加盟费用。";
                                $exchangeFlow['fee'] = 0;
...
//            $this -> error($upload -> getError());
            $this -> agentMsg('图片上传失败！',U('home/support/applyExchange'));
        }else{// 上传成功
    ...
    {
        $exchangeObj = D("Exchange");
        $uid = $_SESSION['uid'];
        $result = $exchangeObj -> reapply($uid);
